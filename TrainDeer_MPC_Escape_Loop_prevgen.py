...
def FirstGen(setSpeed, xCar,mapa,h,n):
    ...
    if(setSpeed<25):
        prevdir = 'GenerationFiles/generations' + str(agent) + '/map_' + str(mapa) + '/xCar' + str(xCar) + '/setSpeed' + str(setSpeed+1)
        all_files = os.listdir(prevdir)
        #look for generation files
        gen_files = []
        gen_pattern = re.compile('.*\.txt')
        for fn in all_files:
            if re.match(gen_pattern, fn):
                gen_files.append(fn)
        print "loading files from setspeed "+str(setSpeed+1)+"....."

        gen_numbers = []
        for genfilename in gen_files:
            stripped = genfilename[0:-4]
            gen_numbers.append(int(stripped[10:]))

        last_gen_number = max(gen_numbers)

        lastgenfilename = 'Generation'+str(last_gen_number)+'.txt'

        print "Using 5 deer from "+lastgenfilename

        lastgenf = open(prevdir+'/'+lastgenfilename,'r')
        lastgengenomes = []
        for line in lastgenf:
            linestrip = line.strip()
            linesplit = line.split(' ')
            lastgengenomes.append(linesplit[1])
        print " using old genomes:"
        for ind in range(0,5):
            Deer10.append(lastgengenomes[ind])
            print(lastgengenomes[ind])
        for ind in range(5,n):
